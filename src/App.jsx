...
function useIsMobile() {
  const [m, setM] = React.useState(typeof window !== "undefined" ? window.innerWidth < 560 : false);
  React.useEffect(() => {
    const onR = () => setM(window.innerWidth < 560);
        ...
        <div style={{ marginTop: 18, borderRadius: 16, overflow: "hidden", background: result.level.bg, color: "#1a2a2a" }}>
          ...
          <div style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
              ...
              <span style={{ fontSize: mobile ? 22 : 28, fontWeight: 800, color: result.level.color }}>{result.nh3.toFixed(4)} <span style={{ fontSize: 14, fontWeight: 500 }}>mg/L</span></span>
            </div>
      ...
      <div style={{ marginTop: 18, background: "#152a35", borderRadius: 14, padding: "14px 18px" }}>
        ...
        {NH3_LEVELS.map((l) => (
          <div key={l.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 0", fontSize: 13 }}>
            ...
            <span style={{ flex: 1, color: "#cddde2" }}>{l.label}</span>
            <span style={{ color: "#8aa3ad", flexShrink: 0, textAlign: "right" }}>{l.max === Infinity ? "≥ 0.10" : `< ${l.max.toFixed(2)}`}</span>
          </div>
        ))}
      </div>
    </>
      ...
      <div style={{ marginTop: 18, background: "#152a35", borderRadius: 14, padding: "14px 18px" }}>
        ...
        {[
          ...
          <div key={x.l} style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 0", fontSize: 13 }}>
            ...
            <span style={{ flex: 1, color: "#cddde2" }}>{x.l}</span>
            <span style={{ color: "#8aa3ad", flexShrink: 0, textAlign: "right", maxWidth: "48%" }}>{x.r}</span>
          </div>
        ))}
      </div>
    </>
...
/* ---------- App ---------- */
/* ---------- Tab 3: Biomassa ---------- */
// Tabel referensi FR (%) terhadap ABW (gram)
const FR_TABLE = [
  { abw: 3, fr: 6.5 },
  { abw: 5, fr: 5.5 },
  { abw: 7.5, fr: 4.2 },
...
function frFromABW(abw) {
  const t = FR_TABLE;
  if (abw <= t[0].abw) return { fr: t[0].fr, extrap: abw < t[0].abw };
  if (abw >= t[t.length - 1].abw) return { fr: t[t.length - 1].fr, extrap: abw > t[t.length - 1].abw };
  for (let i = 0; i < t.length - 1; i++) {
    const a = t[i], b = t[i + 1];
    if (abw >= a.abw && abw <= b.abw) {
      const ratio = (abw - a.abw) / (b.abw - a.abw);
      return { fr: a.fr + ratio * (b.fr - a.fr), extrap: false };
    }
  }
  return { fr: t[t.length - 1].fr, extrap: false };
}

function BiomassTab({ mobile }) {
  const [feed, setFeed] = useState("");
  const [abw, setAbw] = useState("");

  const result = useMemo(() => {
    const fd = parseFloat(feed), w = parseFloat(abw);
    if ([fd, w].some(isNaN)) return null;
    if (w <= 0 || fd < 0) return null;
    const { fr, extrap } = frFromABW(w);
    const biomass = fr > 0 ? (fd / fr) * 100 : 0; // kg
    return { fr, biomass, extrap };
  }, [feed, abw]);
  ...
  return (
    <>
      ...
      {result && (
        <div style={{ marginTop: 18, borderRadius: 16, overflow: "hidden", background: "#e6f0f4", color: "#1a2a2a" }}>
          <div style={{ padding: "18px 20px", background: "#2a6f8e", color: "#fff" }}>
            <div style={{ fontSize: 13, opacity: 0.9 }}>Estimasi Biomassa</div>
            <div style={{ fontSize: mobile ? 24 : 28, fontWeight: 800 }}>{result.biomass.toFixed(1)} <span style={{ fontSize: 16, fontWeight: 500 }}>kg</span></div>
          </div>
          <div style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#46606a", marginBottom: 6 }}>
              <span>FR (feeding rate)</span><span style={{ fontWeight: 700 }}>{result.fr.toFixed(2)} %</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#46606a" }}>
              <span>Biomassa (ton)</span><span style={{ fontWeight: 600 }}>{(result.biomass / 1000).toFixed(3)} ton</span>
            </div>
            {result.extrap && (
              <p style={{ fontSize: 12, color: "#b06a1a", marginTop: 14, marginBottom: 0, lineHeight: 1.5 }}>
                ⚠ ABW di luar rentang tabel (3–29 g). FR memakai nilai batas terdekat, akurasi berkurang.
              </p>
            )}
          </div>
        </div>
      ...
      <div style={{ marginTop: 18, background: "#152a35", borderRadius: 14, padding: "14px 18px" }}>
        <div style={{ fontSize: 12, letterSpacing: 1, textTransform: "uppercase", color: "#5fa8c4", marginBottom: 10 }}>Tabel Referensi FR</div>
        <div style={{ display: "flex", fontSize: 12, color: "#6d8893", paddingBottom: 6, borderBottom: "1px solid #2a4450", fontWeight: 600 }}>
          <span style={{ flex: 1 }}>ABW (g)</span><span style={{ flex: 1, textAlign: "right" }}>FR (%)</span>
        </div>
        {FR_TABLE.map((r, i) => (
          <div key={r.abw} style={{ display: "flex", fontSize: 13, padding: "4px 0", color: "#cddde2" }}>
            <span style={{ flex: 1 }}>{i === FR_TABLE.length - 1 ? "> 33" : r.abw}</span><span style={{ flex: 1, textAlign: "right", color: "#8aa3ad" }}>{r.fr}</span>
          </div>
        ))}
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #2a4450", fontSize: 11, color: "#6d8893", fontStyle: "italic" }}>
          Sumber tabel: Saenphon Chandaeng — Desember 2020
        </div>
      </div>
      <div style={{ marginTop: 14, background: "#152a35", borderRadius: 14, padding: "14px 18px" }}>
        <div style={{ fontSize: 12, letterSpacing: 1, textTransform: "uppercase", color: "#5fa8c4", marginBottom: 10 }}>Catatan Penggunaan · Cek Anco</div>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "5px 0", fontSize: 13, color: "#cddde2", lineHeight: 1.5 }}>
          <span style={{ width: 8, height: 8, borderRadius: 2, background: "#5fa8c4", flexShrink: 0, marginTop: 6 }} />
          <span><strong style={{ color: "#fff" }}>DOC &lt; 45 hari:</strong> pakan di anco/tray 1% sebaiknya habis dalam 1 jam.</span>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "5px 0", fontSize: 13, color: "#cddde2", lineHeight: 1.5 }}>
          <span style={{ width: 8, height: 8, borderRadius: 2, background: "#5fa8c4", flexShrink: 0, marginTop: 6 }} />
          <span><strong style={{ color: "#fff" }}>DOC &gt; 45 hari:</strong> pakan di anco/tray 2% sebaiknya habis dalam 1 jam.</span>
        </div>
        <p style={{ fontSize: 11, color: "#6d8893", marginTop: 10, marginBottom: 0, lineHeight: 1.5 }}>
          Gunakan cek anco sebagai kalibrasi: bila pakan tidak habis pada waktu di atas, FR & estimasi biomassa cenderung terlalu tinggi — turunkan pakan.
        </p>
      </div>
    </>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0e1c24", fontFamily: "system-ui, sans-serif", padding: mobile ? "16px 12px" : "24px 16px", color: "#e8eef0" }}>
      <div style={{ maxWidth: 520, margin: "0 auto", width: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, gap: 12 }}>
          <div>
            <div style={{ fontSize: mobile ? 10 : 12, letterSpacing: 1.5, textTransform: "uppercase", color: "#5fa8c4", marginBottom: 6 }}>BILLY · SUMBER HATCHERY BANGKA</div>
            <h1 style={{ fontSize: mobile ? 20 : 26, fontWeight: 700, margin: 0, lineHeight: 1.15 }}>Kalkulator Kualitas Air</h1>
          </div>
          <img src={LOGO} alt="Sumber Hatchery Bangka" style={{ width: mobile ? 76 : 144, height: mobile ? 76 : 144, borderRadius: mobile ? 12 : 16, objectFit: "cover", flexShrink: 0, boxShadow: "0 2px 10px rgba(0,0,0,.3)" }} />
        </div>
        <div style={{ display: "flex", gap: 6, background: "#152a35", padding: 5, borderRadius: 12, marginBottom: 20 }}>
          {tabs.map((t, i) => (
            <button key={t} onClick={() => setTab(i)}
              style={{ flex: 1, padding: mobile ? "9px 4px" : "10px 8px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: mobile ? 12 : 13, fontWeight: 600,
                background: tab === i ? "#2a6f8e" : "transparent", color: tab === i ? "#fff" : "#8aa3ad", transition: "background .15s" }}>
              {t}
            </button>
          ))}
        </div>
        {tab === 0 ? <AmmoniaTab mobile={mobile} /> : tab === 1 ? <NPTab mobile={mobile} /> : <BiomassTab mobile={mobile} />}
